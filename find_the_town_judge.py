...
is a rumor that one of these people is secretly the town 
judge.
...
#trusting, trusted = map(list, zip(*trust))
...
class Solution:
    def findJudge(self, N: int, trust: List[List[int]]) -> int:
        ...
        trust_pairs = [[0, 0] for i in range(N+1)]
        
        for i in range(len(trust)):
            trust_pairs[trust[i][0]][0] += 1
            trust_pairs[trust[i][1]][1] += 1
            
        for i in range(len(trust_pairs)):
            if trust_pairs[i][0] == 0 and trust_pairs[i][1] == N-1:
                return i
